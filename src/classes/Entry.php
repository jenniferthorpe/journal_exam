...
class Entry extends Mapper {
  ...
  public function updateEntry($content, $title, $entryID, $userID){
    $statement = $this->db->prepare("UPDATE entries SET title = :title, content = :content WHERE userID = :userID AND entryID = :entryID");
    $statement->execute([
      ':content' => $content,
      ':title' => $title,
      ':entryID' => $entryID,
